...
% PROVA
% B = imread("Background.png"); % BACKGROUND prova
% I = imread("Soggetto.png"); % SOGGETTO prova

outputFolder = 'E1'; % cartella in cui salvare le immagini acquisite

if exist(outputFolder,"dir") rmdir(outputFolder, 's'); end
mkdir(outputFolder);

imwrite(B,fullfile(outputFolder, 'E1_0.bmp')); % sfondo
imwrite(I,fullfile(outputFolder, 'E1_1.bmp')); % frame iniziale
...
if ~isnan(distanzaPolsi), width = 2*distanzaPolsi; end
if(isnan(distanzaPolsi)), width = 4*distanzaSpalle; end
cen = centroid(1);
perc = 0.8;
RW = [fix(cen - (perc)*width/2) fix(cen + (perc)*width/2)]; %lower and upper limits
target = [randi(RW, 1) centroid(2)]; %target random initialisation
...
tic
while score<TargetNumber && time <= maxTime %non appena una condzione non si verifica più, esco dal ciclo
    ...
    I = snapshot(cam); % SOGGETTO real-time
    imwrite(I, fullfile(outputFolder, ['E1_' num2str(giro_while) '.bmp'])); 
    saveTarget(giro_while,:)=target; % mi salvo il target per ogni frame
    I = INVERT_IMG(I);
    I = imresize(I, [round(size(I,1)/3) round(size(I,2)/3)]);
    netInput = im2single(I)-0.5; % im2single mi trasforma in un range tra 0 e 1, togliendo 0,5 ottengo range da -0,5 a +0,5
    netInput = netInput(:,:, [3 2 1]); % l'immagine tradizionle è rgb, ma la rete si aspetta bgr, quindi devo invertire l'ordine dei canali
    ...
    TH = 2; % soglia di tolleranza per allineamento del centroide con il target
    ...
    if abs(centroid(1)-target(1))< TH % TARGET RAGGIUNTO!
        ...
        minDistance = 4; 
        % Ri-inizializzo per un nuovo esercizio
        newTargetInitialized = false;
        while ~newTargetInitialized
            target = ([randi(RW, 1) centroid(2)]); % target random initialisation        
            if abs(target(1) - centroid(1)) >= minDistance
                newTargetInitialized = true;
            end
        end
...
%% Clinical report
if exist(fullfile(outputFolder, 'clinical_report_I.xls'), "file")
    delete('clinical_report_I.xls')
end

C = {'Type of excercise: IMPROVE YOUR BALANCE','';...
     "RW (% of subject's width)", perc*100;...
     'Speed (pixel/s)', speed;...
     'Total amount of time (s)', time_end(end);...
     'Number of target achived (out of 10)', score};
writecell(C,fullfile(outputFolder, 'clinical_report_I.xls'));
