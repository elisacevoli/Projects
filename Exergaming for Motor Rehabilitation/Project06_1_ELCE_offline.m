...
% OFFLINE
B = imread(fullfile(outputFolder, 'E1_0.bmp')); % BACKGROUND
I = imread(fullfile(outputFolder, 'E1_1.bmp')); % SOGGETTO
...
if ~isnan(distanzaPolsi), width = 1.5*distanzaPolsi; end
if(isnan(distanzaPolsi)), width = 3*distanzaSpalle; end
cen = centroid(1);
perc = 0.8;
...
target = [targetE1(1,1) targetE1(1,2)]; % OFFLINE
...
for i = 1:numBmpFiles-1
    ...
    I = imread(fullfile(outputFolder, ['E1_', num2str(i), '.bmp'])); % SOGGETTO offline
    target = [targetE1(i,1) targetE1(i,2)]; % OFFLINE
    I = INVERT_IMG(I);
    I = imresize(I, [round(size(I,1)/3) round(size(I,2)/3)]);
    netInput = im2single(I)-0.5; % im2single mi trasforma in un range tra 0 e 1, togliendo 0,5 ottengo range da -0,5 a +0,5
    netInput = netInput(:,:, [3 2 1]); % l'immagine tradizionle è rgb, ma la rete si aspetta bgr, quindi devo invertire l'ordine dei canali
    ...
    if abs(centroid(1)-target(1))< TH % TARGET RAGGIUNTO!
        ...
        minDistance = 4; 
        % Ri-inizializzo per un nuovo esercizio (real-time)
