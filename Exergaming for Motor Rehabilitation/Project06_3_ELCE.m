...
outputFolder = 'E3'; % cartella in cui salvare le immagini acquisite

if exist(outputFolder,"dir") rmdir(outputFolder, 's'); end
mkdir(outputFolder);

imwrite(B,fullfile(outputFolder, 'E3_0.bmp')); % sfondo
imwrite(I,fullfile(outputFolder, 'E3_1.bmp')); % frame iniziale
...
if ~isnan(distanzaPolsi), width = 4*distanzaPolsi; end
if(isnan(distanzaPolsi)), width = 8*distanzaSpalle; end
cen = centroid(2);
down_th = 20;
RW = [fix(cen) fix(cen + down_th)]; %lower and upper limits
target = [centroid(1) randi(RW, 1)]; %target random initialisation
...
CoM_start(1) = centr_iniziale(2); % coordinata y del centroide
time_start(1) = 0;
...
while score<TargetNumber && time <= maxTime %non appena una condzione non si verifica più, esco dal ciclo
    ...
    I = snapshot(cam); % SOGGETTO real-time
    imwrite(I, fullfile(outputFolder, ['E3_' num2str(giro_while) '.bmp']));
    saveTarget(giro_while,:)=target;
    I = INVERT_IMG(I);
    I = imresize(I, [round(size(I,1)/3) round(size(I,2)/3)]);
    netInput = im2single(I)-0.5; % im2single mi trasforma in un range tra 0 e 1, togliendo 0,5 ottengo range da -0,5 a +0,5
    netInput = netInput(:,:, [3 2 1]); % l'immagine tradizionle è rgb, ma la rete si aspetta bgr, quindi devo invertire l'ordine dei canali
    ...
    TH = 3;
    if abs(centroid(2)-target(2))< TH % TARGET RAGGIUNTO!
        ...
        time_end(score) = toc;
        CoM_end(score) = centroid(2); % coordinata y del centroide
...
%% Clinical report
if exist(fullfile(outputFolder, 'clinical_report_III.xls'), "file")
    delete('clinical_report_III.xls')
end

C = {'Type of excercise: IMPROVE YOUR STRENGHT OF LOWER LIMBS','';...
     "RW (pixels)", down_th;...
     'Speed (pixel/s)', speed;...
     'Total amount of time (s)', time_end(end);...
     'Number of target achived (out of 10)', score};
writecell(C,fullfile(outputFolder, 'clinical_report_III.xls'));
